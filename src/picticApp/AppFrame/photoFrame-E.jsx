...
export default function PhotoFrameE({ images, photo, stickers = [], setStickers  }) {
  return (
    <div
      className={`relative p-4 shadow w-68 md:w-76 ${images.layout}`}
      style={{
      ...
      <div className="grid grid-cols-2 gap-2">
        {photo.slice(0, 6).map((src, i) => (
          <img
            ...
            src={src}
            alt={`photo-${i}`}
            className="w-full h-32 md:h-34 object-cover rounded-md"
          />
        ))}
      </div>
